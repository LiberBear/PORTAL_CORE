...
BX.InviteDialog.showMessage = function(strMessageText, strWarningText)
{
	...
	if (BX('intranet-dialog-tabs'))
	{
		...
		BX('intranet-dialog-tabs').parentNode.appendChild(BX.create("table", {
			...
			props: {
				style: "min-height:100px; min-width: 420px;   margin: 15px 0 9px 0;"
			}
		}));
...
BX.InviteDialog.getEmail2 = function()
{
	...
	)
	{
		...
		if (typeof BX('ADD_MAILBOX_DOMAIN_connect').options != 'undefined')
		{
			var serviceID = BX("ADD_MAILBOX_DOMAIN_connect").options[BX("ADD_MAILBOX_DOMAIN_connect").selectedIndex].getAttribute('data-service-id');
		}
		else
		{
			var serviceID = BX("ADD_MAILBOX_SERVICE_connect").value;
		}
			...
			case "invite-dialog-add-button-submit":
				...
				)
				{
					...
					else if (BX('ADD_MAILBOX_ACTION').value == 'connect')
					{
						...
						if (typeof BX('ADD_MAILBOX_DOMAIN_connect').options != 'undefined')
						{
							for (i = 0; i < BX('ADD_MAILBOX_USER_connect').options.length; i++)
							{
								if (BX('ADD_MAILBOX_USER_connect').options[i].selected)
								{
									obRequestData.ADD_MAILBOX_SERVICE = BX('ADD_MAILBOX_DOMAIN_connect').options[i].getAttribute('data-service-id');
									break;
								}
							}
						}
						...
						{
							obRequestData.ADD_MAILBOX_SERVICE = BX('ADD_MAILBOX_SERVICE_connect').value;
						}
					}
				}
