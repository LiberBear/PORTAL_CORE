						...
						html += ([
							'<div class="mobile-grid-field-select-user-item-outer">',
								'<div class="mobile-grid-field-select-user-item">',
									...
									'<span onclick="BXMobileApp.PageManager.loadPageBlank({url: \'' +  BX.message("interface_form_user_url").replace("#ID#", user["ID"]) + '\',bx24ModernStyle : true});">' + BX.util.htmlspecialchars(user["NAME"]) + '</span>',
								'</div>',
							'</div>'
	...
	BX.Mobile.Grid.Form.prototype = {
		...
		bindElement : function(node) {
			...
			if (BX(node))
			{
				...
				{
					result = BX.Disk.UF.getByName(node.value);
				}
			}
			return result;
		},
