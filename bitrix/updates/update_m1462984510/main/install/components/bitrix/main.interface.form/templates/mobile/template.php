								...
								case 'select-user':
									...
									if (is_array($field["item"]))
									{
										...
										?><div class="mobile-grid-field-select-user-item-outer"><div class="mobile-grid-field-select-user-item"><?
											...
											<span onclick="BXMobileApp.PageManager.loadPageBlank({url: '<?=str_replace("#ID#", $item["id"], $userUrl)?>',bx24ModernStyle : true});"><?=htmlspecialcharsbx($item["name"])?></span>
										</div></div><?
									}
									$users = ob_get_clean();
...
?></div><?
if (isset($arParams["BUTTONS"]) && strtolower($arParams["BUTTONS"]) == "app")
{
