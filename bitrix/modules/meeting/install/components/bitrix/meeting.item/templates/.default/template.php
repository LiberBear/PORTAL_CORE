...
{
	var q = BX('task-' + task.id);
	BX.onCustomEvent('onTaskListTaskAdd', [task, q ? function () {
		q.style.display = '';
	} : function() {
		var w = BX.showWait(BX('meeting_item_tasks', true));
		BX.ajax.get('<?=CUtil::JSEscape($APPLICATION->GetCurPageParam('MEETING_TASKS_RELOAD=Y', array('MEETING_ITEM_EDIT', 'MEETING_ITEM_VIEW', 'MEETING_TASKS_RELOAD', 'MEETING_ITEM_COMMENTS')))?>', function(data)
				{
...
BX.addCustomEvent('onTaskListTaskAdd', function(obTask, cb) {
	if (!BX('meeting_task_' + obTask.id))
	{
		document.forms.meeting_item_edit.appendChild(BX.create('INPUT', {
			attrs: {type: 'hidden', name: 'ITEM_TASKS[]', value: obTask.id, id: 'meeting_task_' + obTask.id}
		}));
		saveData(cb);
	}
});
