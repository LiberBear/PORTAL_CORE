...
class DropboxHandler extends DocumentHandler
{
	...
	public function getFileMetadata(FileData $fileData)
	{
		...
		if($http->get(self::API_URL . "/metadata/auto/{$fileId}?list=false") === false)
		{
	...
	public function downloadPartFile(FileData $fileData, $startRange, $chunkSize)
	{
		...
		if($http->download(self::API_CONTENT_URL . "/files/auto/{$fileId}", $fileData->getSrc()) === false)
		{
			$errorString = implode('; ', array_keys($http->getError()));
			$this->errorCollection->add(array(
	...
	public function listFolder($path, $folderId)
	{
		...
		foreach($items['contents'] as $item)
		{
			...
			$reformatItems[$pseudoId] = array(
				'id' => $pseudoId,
				'name' => bx_basename($item['path']),
				'type' => $isFolder? 'folder' : 'file',

				'size' => $isFolder? '' : \CFile::formatSize($item['bytes']),
				'sizeInt' => $isFolder? '' : $item['bytes'],
				'modifyBy' => '',
	...
	protected function normalizeMetadata($metaData)
	{
		return array(
			...
			'name' => bx_basename($metaData['path']),
			'size' => $metaData['bytes'],
