			...
			|| $this->currentPhase === DealConversionPhase::QUOTE_CREATION)
		{
			...
			if($entityTypeID === \CCrmOwnerType::Invoice)
			{
				...
				if (isset($fields['UF_DEAL_ID']) && $fields['UF_DEAL_ID'] > 0)
					$requisiteEntityList[] = array('ENTITY_TYPE_ID' => \CCrmOwnerType::Deal, 'ENTITY_ID' => $fields['UF_DEAL_ID']);
				if (isset($fields['UF_QUOTE_ID']) && $fields['UF_QUOTE_ID'] > 0)
					$requisiteEntityList[] = array('ENTITY_TYPE_ID' => \CCrmOwnerType::Quote, 'ENTITY_ID' => $fields['UF_QUOTE_ID']);
				...
				{
					EntityLink::register(
						\CCrmOwnerType::Invoice, $entityID, $requisiteIdLinked, $bankDetailIdLinked
					);
				}
			...
			else//if($entityTypeID === \CCrmOwnerType::Quote)
			{
				...
				if (isset($fields['DEAL_ID']) && $fields['DEAL_ID'] > 0)
					$requisiteEntityList[] = array('ENTITY_TYPE_ID' => \CCrmOwnerType::Deal, 'ENTITY_ID' => $fields['DEAL_ID']);
				...
				{
					EntityLink::register(
						\CCrmOwnerType::Quote, $entityID, $requisiteIdLinked, $bankDetailIdLinked
					);
				}
				unset($requisiteIdLinked);
