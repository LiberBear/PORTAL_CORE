	...
	public static function upsert(array $data)
	{
		...
		if($connection instanceof Main\DB\MysqlCommonConnection)
		{
			$connection->queryExecute( /** @lang MySQL */
				"INSERT INTO b_crm_requisite_link (ENTITY_TYPE_ID, ENTITY_ID, REQUISITE_ID, BANK_DETAIL_ID)".PHP_EOL.
				"  VALUES ({$entityTypeId}, {$entityId}, {$requisiteId}, {$bankDetailId})".PHP_EOL.
				"    ON DUPLICATE KEY UPDATE REQUISITE_ID = {$requisiteId}, BANK_DETAIL_ID = {$bankDetailId}".PHP_EOL
			);
		}
		elseif($connection instanceof Main\DB\MssqlConnection)
		{
			...
			if(is_array($dbResult->fetch()))
			{
				$connection->queryExecute( /** @lang TSQL */
					...
					"  SET REQUISITE_ID = {$requisiteId}, BANK_DETAIL_ID = {$bankDetailId}".PHP_EOL.
					"WHERE ENTITY_TYPE_ID = {$entityTypeId} AND ENTITY_ID = {$entityId}".PHP_EOL
				);
			}
			else
		...
		elseif($connection instanceof Main\DB\OracleConnection)
		{
			$connection->queryExecute( /** @lang Oracle */
				...
				"USING (SELECT {$entityTypeId} ENTITY_TYPE_ID, {$entityId} ENTITY_ID, {$requisiteId} REQUISITE_ID, {$bankDetailId} BANK_DETAIL_ID FROM dual) source".PHP_EOL.
				"ON (".PHP_EOL.
				...
				"  UPDATE SET B_CRM_REQUISITE_LINK.REQUISITE_ID = {$requisiteId}, B_CRM_REQUISITE_LINK.BANK_DETAIL_ID = {$bankDetailId}".PHP_EOL.
				"WHEN NOT MATCHED THEN".PHP_EOL.
				"  INSERT (ENTITY_TYPE_ID, ENTITY_ID, REQUISITE_ID, BANK_DETAIL_ID)".PHP_EOL.
				"  VALUES ({$entityTypeId}, {$entityId}, {$requisiteId}, {$bankDetailId})".PHP_EOL
			);
		}
		else
