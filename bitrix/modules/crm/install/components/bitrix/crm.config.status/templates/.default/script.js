BX.CrmConfigStatusClass = (function ()
{
	var CrmConfigStatusClass = function (parameters)
	{
		...
		this.twoPhaseSettings = {
			'DEAL_STAGE': {'question': BX.message('CRM_STATUS_DELETE_FIELD_QUESTION_DEAL_STAGE')},
			'STATUS': {'question': BX.message('CRM_STATUS_DELETE_FIELD_QUESTION_STATUS')},
			'QUOTE_STATUS': {'question': BX.message('CRM_STATUS_DELETE_FIELD_QUESTION_QUOTE_STATUS')},
			'INVOICE_STATUS': {'question': BX.message('CRM_STATUS_DELETE_FIELD_QUESTION_INVOICE_STATUS')}
		};
	...
	CrmConfigStatusClass.prototype.selectTab = function(tabId)
	{
		...
		if(tabId in this.twoPhaseSettings)
		{
			AmCharts.handleLoad();
		}
	};
	...
	CrmConfigStatusClass.prototype.openPopupBeforeDeleteField = function(fieldId)
	{
		...
		var content;
		if(this.entityId in this.twoPhaseSettings)
		{
			content = BX.create('p', {
				props: {className: 'bx-crm-popup-paragraph'},
				html: this.twoPhaseSettings[this.entityId]['question']
			});
		}
		else
		{
			content = BX.create('p', {
				props: {className: 'bx-crm-popup-paragraph'},
				html: BX.message('CRM_STATUS_DELETE_FIELD_QUESTION')
			});
		}
	...
	CrmConfigStatusClass.prototype.recalculateSort = function()
	{
		...
		if(this.initAmChart && (this.entityId in this.twoPhaseSettings))
		{
