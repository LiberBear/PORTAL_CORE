...
<div id="crm-container" class="crm-container">
	...
	<div id="status_box" class="crm-transaction-menu">
		<? foreach($arResult['HEADERS'] as $entityId => $headerName): ?>
			...
			<a href="javascript:void(0)" id="status_tab_<?=$entityId?>" class="status_tab <?=$tabActive?>"
			   onclick="BX['<?=$jsClass?>'].selectTab('<?=$entityId ?>');">
				<?=$headerName?>
			</a>
		<? endforeach; ?>
	</div>
	...
	<div class="crm-transaction-stage">
		<? foreach($arResult['HEADERS'] as $entityId => $headerName): ?>
			<? $maxSort = 0; ?>
			<? if(array_key_exists($entityId, $arResult['ENTITY'])): ?>
			...
			<div id="content_<?=$entityId?>"
				...
				<div class="transaction-stage transaction-initial-stage">
					...
					<?
						$iconClass = '';
						$blockClass = '';
						$colorValue = $initialFields[$entityId]['COLOR'] ? $initialFields[$entityId]['COLOR'] : '#4C99DA';
				...
				<div id="extra-storage-<?=$entityId?>" class="transaction-stage droppable">
					...
					<? foreach($extraFields[$entityId] as $field): ?>
						...
						<?
							$blockClass = '';
							$iconClass = '';
							$colorValue = $field['COLOR'] ? $field['COLOR'] : '#4C99DA';
							$style = 'background:'.$colorValue.';';
							$style .= 'color:'.getColorText($colorValue, $iconClass, $blockClass).';';
						?>
					...
					<a href="javascript:void(0)" onclick="BX['<?=$jsClass?>'].addField(this);"
					   class="transaction-stage-addphase draghandle" data-space="main">+
						<span><?=GetMessage('CRM_STATUS_ADD_'.$entityId)?></span>
					</a>
				</div>
					...
					<div class="transaction-stage-final-column">
						...
						<?
							...
							$colorValue = $finalFields[$entityId]['SUCCESSFUL']['COLOR'] ?
								$finalFields[$entityId]['SUCCESSFUL']['COLOR'] : '#96B833';
							$style = 'background:'.$colorValue.';';
							$style .= 'color:'.getColorText($colorValue, $iconClass, $blockClass).';';
						?>
					...
					<div class="transaction-stage-final-column">
						...
						<?
							...
							$colorValue = $finalFields[$entityId]['UNSUCCESSFUL']['COLOR'] ?
								$finalFields[$entityId]['UNSUCCESSFUL']['COLOR'] : '#F54819';
							$style = 'background:'.$colorValue.';';
							$style .= 'color:'.getColorText($colorValue, $iconClass, $blockClass).';';
						?>
						...
						<div id="final-storage-<?=$entityId?>" class="transaction-stage transaction-stage-failure droppable">
							...
							<? foreach($extraFinalFields[$entityId] as $field): ?>
								...
								<?
									$blockClass = '';
									$iconClass = '';
									$colorValue = $field['COLOR'] ? $field['COLOR'] : '#F54819';
									$style = 'background:'.$colorValue.';';
									$style .= 'color:'.getColorText($colorValue, $iconClass, $blockClass).';';
								?>
									...
									<table class="crm-previously-table">
										...
										<tr id="previously-scale-final-success-<?=$entityId?>">
											<td style="background:<?= (isset($finalFields[$entityId]['SUCCESSFUL']['COLOR'])) ?
												$finalFields[$entityId]['SUCCESSFUL']['COLOR'] : '#96B833'?>">&nbsp;</td>
										</tr>
									...
									<table class="crm-previously-table">
										...
										<tr id="previously-scale-<?=$entityId?>">
											<td data-scale-type="main"
												...
												$initialFields[$entityId]['COLOR'] : '#4C99DA'?>">&nbsp;</td>

											<?foreach($extraFields[$entityId] as $field):?>
												<td data-scale-type="main"
													style="background:<?= (isset($field['COLOR'])) ?
													$field['COLOR'] : '#4C99DA'?>">&nbsp;</td>
											<?endforeach;?>
									...
									<table class="crm-previously-table">
										...
										<tr id="previously-scale-final-un-success-<?=$entityId?>">
											<td style="background:<?=(isset($finalFields[$entityId]['UNSUCCESSFUL']['COLOR'])) ?
												$finalFields[$entityId]['UNSUCCESSFUL']['COLOR'] : '#F54819'?>">&nbsp;</td>
											<?foreach($extraFinalFields[$entityId] as $field):?>
												<td style="background:<?= (isset($field['COLOR'])) ?
													$field['COLOR'] : '#F54819'?>">&nbsp;</td>
											<?endforeach;?>
										</tr>
...
<script type="text/javascript">
	BX.ready(function(){
		...
		BX.message({
			CRM_STATUS_NEW_DEAL_STAGE: '<?= GetMessageJS('CRM_STATUS_NEW_DEAL_STAGE') ?>',
			CRM_STATUS_NEW_STATUS: '<?= GetMessageJS('CRM_STATUS_NEW_STATUS') ?>',
			CRM_STATUS_NEW_QUOTE_STATUS: '<?= GetMessageJS('CRM_STATUS_NEW_QUOTE_STATUS') ?>',
			CRM_STATUS_NEW_INVOICE_STATUS: '<?= GetMessageJS('CRM_STATUS_NEW_INVOICE_STATUS') ?>',
			...
			CRM_STATUS_DELETE_FIELD_QUESTION_DEAL_STAGE: '<?= GetMessageJS('CRM_STATUS_DELETE_FIELD_QUESTION_DEAL_STAGE') ?>',
			CRM_STATUS_DELETE_FIELD_QUESTION_STATUS: '<?= GetMessageJS('CRM_STATUS_DELETE_FIELD_QUESTION_STATUS') ?>',
			CRM_STATUS_DELETE_FIELD_QUESTION_QUOTE_STATUS: '<?= GetMessageJS('CRM_STATUS_DELETE_FIELD_QUESTION_QUOTE_STATUS') ?>',
			CRM_STATUS_DELETE_FIELD_QUESTION_INVOICE_STATUS: '<?= GetMessageJS('CRM_STATUS_DELETE_FIELD_QUESTION_INVOICE_STATUS') ?>',
		...
		BX['<?=$jsClass?>'] = new BX.CrmConfigStatusClass({
			...
			entityId: BX("ACTIVE_TAB").value.replace('status_tab_', ''),
