...
class CCrmRequisiteEditComponent extends \CBitrixComponent
{
	...
	protected function parseParams()
	{
		...
		/*else
		{
			$this->errors[] = GetMessage('CRM_REQUISITE_EDIT_ERR_PRESET_NOT_SELECTED', array('#ID#' => $this->elementId));
			return false;
		}*/
	...
	protected function prepareResult()
	{
					...
					if ($fieldName === EntityRequisite::ADDRESS)
					{
						...
						$rqFields[] = array(
							...
							'componentParams' => array(
								...
								'DATA' => EntityRequisite::getAddresses($this->elementId)
							)
						);
					}
					else
		...
		if (!empty($rqFieldsSortedIndex))
		{
			...
			foreach ($rqFieldsSortedIndex as $rqFieldIndex)
				$this->arResult['FIELDS']['tab_1'][] = &$rqFields[$rqFieldIndex];
			unset($rqFieldIndex);
		}
		...
		foreach ($this->arResult['FIELDS']['tab_1'] as &$fieldInfo)
		{
			...
			{
				if (isset($this->presetFieldTitles[$fieldInfo['id']])
		...
		foreach ($this->arResult['FIELDS'] as $tabId => $tabFields)
		{
			...
			foreach ($tabFields as $fieldIndex => $field)
			{
				...
				if (isset($this->fieldsAllowed[$fieldId]) && !isset($this->presetFields[$fieldId]))
				{
					if (isset($this->fieldsOfActivePresets[$fieldId])
