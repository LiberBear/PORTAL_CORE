...
BX.Crm.PresetFieldListManagerClass = (function ()
{
	...
	PresetFieldListManagerClass.prototype = {
		...
		editField: function (fieldId)
		{
			...
			this.dlg.popup = new BX.PopupWindow(
				...
				null,
				{
					...
					titleBar:
					{
						content: this._prepareFieldEditDialogTitle()
					},
					events:
					{
		...
		_prepareFieldEditDialogTitle: function()
		{
			return BX.create(
				'SPAN',
				{
					html: BX.util.htmlspecialchars(
						(this.dlg.fieldId > 0) ?
							this.getMessage('fieldEditDialogTitle') : this.getMessage('fieldAddDialogTitle')
					),
