		...
		if (strlen($errorMessage) <= 0)
		{
			...
			if (strlen($errorMessage) <= 0)
			{
				...
				if (isset($_POST['PS_ACTION_FIELDS_LIST']) && strlen($_POST['PS_ACTION_FIELDS_LIST']) > 0)
				{
					...
					CCrmPaySystem::rewritePSCorrByRqSource($personTypeId, $arPsActFields);
...
CCrmPaySystem::rewritePSCorrByRqSource($ptID, $arResult['PS_ACT_FIELDS']);
...
foreach ($arResult['PS_ACT_FIELDS'] as $idCorr => $arCorr)
{
	if(isset($arPaySys['ACTION']['PARAMS'][$idCorr])
		...
		&& $arPaySys['ACTION']['PARAMS'][$idCorr]['TYPE'] === 'REQUISITE'
		&& isset($arPaySys['ACTION']['PARAMS'][$idCorr]['VALUE'])
	)
	{
		$fieldName = trim(strval($arPaySys['ACTION']['PARAMS'][$idCorr]['VALUE']));
		if (!empty($fieldName))
