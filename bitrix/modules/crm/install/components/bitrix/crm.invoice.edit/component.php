...
if ($conversionWizard !== null || $bEdit || $bCopy)
{
	...
	if ($bEdit || $bCopy)
	{
		if ($arParams['ELEMENT_ID'] > 0)
			$requisiteEntityList[] = array('ENTITY_TYPE_ID' => CCrmOwnerType::Invoice, 'ENTITY_ID' => $arParams['ELEMENT_ID']);
	}
	else if ($conversionWizard !== null)
	{
		if ($bCreateFromDeal || $conversionWizard instanceof \Bitrix\Crm\Conversion\DealConversionWizard)
		{
			if (isset($arFields['UF_DEAL_ID']) && $arFields['UF_DEAL_ID'] > 0)
				$requisiteEntityList[] = array('ENTITY_TYPE_ID' => CCrmOwnerType::Deal, 'ENTITY_ID' => $arFields['UF_DEAL_ID']);
			if (isset($arFields['UF_QUOTE_ID']) && $arFields['UF_QUOTE_ID'] > 0)
				$requisiteEntityList[] = array('ENTITY_TYPE_ID' => CCrmOwnerType::Quote, 'ENTITY_ID' => $arFields['UF_QUOTE_ID']);
		}
		else
		{
			if (isset($arFields['UF_QUOTE_ID']) && $arFields['UF_QUOTE_ID'] > 0)
...
else
{
	if ($bProcessPost)
	{
		...
		if (isset($_POST['save']) || isset($_POST['saveAndView']) || isset($_POST['saveAndAdd']) || isset($_POST['apply']) || isset($_POST['continue']) || $bAjaxSubmit)
		{
			...
			$clientRequisiteId = isset($_POST['CLIENT_REQUISITE_ID']) ? (int)$_POST['CLIENT_REQUISITE_ID'] : 0;
			if ($clientRequisiteId < 0)
				$clientRequisiteId = 0;
			$clientBankDetailId = isset($_POST['CLIENT_BANK_DETAIL_ID']) ? (int)$_POST['CLIENT_BANK_DETAIL_ID'] : 0;
				...
				if ($bSuccess)
				{
					if ($requisiteIdLinked > 0)
					{
						\Bitrix\Crm\Requisite\EntityLink::register(
							CCrmOwnerType::Invoice, $arResult['ELEMENT']['ID'], $requisiteIdLinked, $bankDetailIdLinked
						);
					}
					else
					{
...
if (CCrmContact::CheckReadPermission() && CCrmCompany::CheckReadPermission())
{
	...
	$arResult['FIELDS']['tab_1'][] = array(
		...
		'componentParams' => array(
			...
			'NAME_TEMPLATE' => $arParams['NAME_TEMPLATE'],
			'REQUISITE_INPUT_NAME' => 'CLIENT_REQUISITE_ID',
			'BANK_DETAIL_INPUT_NAME' => 'CLIENT_BANK_DETAIL_ID',
