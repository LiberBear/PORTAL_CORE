		...
		if(!$skipAssocEntity && $typeID > 0 && $associatedEntityID <= 0)
		{
			switch($typeID)
			{
				...
				case CCrmActivityType::Meeting:
				{
					...
					// Check for settings
					$displayCompleted = ActivitySettings::getValue(
						$typeID === CCrmActivityType::Call
						? ActivitySettings::KEEP_COMPLETED_CALLS
						: ActivitySettings::KEEP_COMPLETED_MEETINGS
					);
	...
	public static function Delete($ID, $checkPerms = true, $regEvent = true, $options = array())
	{
		...
		if(!$skipAssocEntity && isset($ary['TYPE_ID']) && isset($ary['ASSOCIATED_ENTITY_ID']))
		{
			switch(intval($ary['TYPE_ID']))
			{
				...
				case CCrmActivityType::Task:
					{
						if(!isset($options['SKIP_TASKS']) || $options['SKIP_TASKS'] === false)
						{
							self::DeleteTask($ary);
						}
					}
...
//			}
			...
			$end = isset($fields['END_TIME']) && $fields['END_TIME'] !== '' ? $fields['END_TIME'] : null;

			if ($typeID === CCrmActivityType::Task && $end !== null)
			{
				$fields['DEADLINE'] = $end;
			}
			elseif ($typeID !== CCrmActivityType::Task && $start !== null)
			{
		...
		else//if($action == 'UPDATE')
		{
			...
			$start = isset($fields['START_TIME']) ? $fields['START_TIME'] : null;
			$end = isset($fields['END_TIME']) ? $fields['END_TIME'] : null;
	...
	private static function SetFromTask($taskID, &$arTaskFields, &$arFields)
	{
		...
		if(isset($arTaskFields['DEADLINE']) || isset($arTaskFields['CLOSED_DATE']) || isset($arTaskFields['END_DATE_PLAN']))
		{
			...
			if($end === null && isset($arTaskFields['END_DATE_PLAN']) && $arTaskFields['END_DATE_PLAN'] !== false)
			{
				$end = $arTaskFields['END_DATE_PLAN'];
			}
		}

		$arFields['START_TIME'] = $start !== null ? $start : '';
		$arFields['END_TIME'] = $end !== null ? $end : '';
		if($arFields['START_TIME'] === '' && $arFields['END_TIME'] !== '')
		{
			$arFields['START_TIME'] = $arFields['END_TIME'];
		}
		elseif($arFields['END_TIME'] === '' && $arFields['START_TIME'] !== '')
		{
			$arFields['END_TIME'] = $arFields['START_TIME'];
		}
		...
		if(self::TryResolveUserFieldOwners($arTaskOwners, $arOwnerData, CCrmUserType::GetTaskBindingField()))
		{
			foreach($arOwnerData as $arOwnerInfo)
			{
				$arFields['BINDINGS'][] = array(
					...
					'OWNER_ID' => (int)$arOwnerInfo['OWNER_ID']
				);
			}
		}

		if(!empty($arFields['BINDINGS']))
		{
			//Check for owner change
			$ownerTypeID = isset($arFields['OWNER_TYPE_ID']) ? (int)$arFields['OWNER_TYPE_ID'] : CCrmOwnerType::Undefined;
			$ownerID = isset($arFields['OWNER_ID']) ? (int)$arFields['OWNER_ID'] : 0;
			$ownerIsFound = false;
			foreach($arFields['BINDINGS'] as $binding)
			{
			...
			if(!$ownerIsFound)
			{
				$binding = $arFields['BINDINGS'][0];
				$arFields['OWNER_TYPE_ID'] = $binding['OWNER_TYPE_ID'];
				$arFields['OWNER_ID'] = $binding['OWNER_ID'];
			}
		}
	}
	...
	protected static function SaveCalendarEvent(&$arFields)
	{
		...
		if($entityID > 0)
		{
			...
			if($associatedEntityID > 0 && $associatedEntityID !== $entityID)
			{
				$keepReassigned = ActivitySettings::getValue(
					$typeID === CCrmActivityType::Call
						? ActivitySettings::KEEP_REASSIGNED_CALLS
						: ActivitySettings::KEEP_REASSIGNED_MEETINGS
				);

				if(!$keepReassigned)
				{
					CCalendarEvent::Delete(array('id' => $associatedEntityID, 'bMarkDeleted' => true));
				}
			}
		}
	...
	protected static function DeleteCalendarEvent(&$arFields)
	{
		...
		self::$IGNORE_CALENDAR_EVENTS = true;
		CCalendarEvent::Delete(array('id' => $assocEntityID, 'bMarkDeleted' => true));
		self::$IGNORE_CALENDAR_EVENTS = false;
		return true;
	}
	...
	private static function RegisterLiveFeedEvent(&$arFields)
	{
		...
		if ($arTask)
		{
			...
			{
				$eventID = CCrmLiveFeed::ConvertTasksLogEvent(array(
					"LOG_ID" => $arLog["ID"],
					"ACTIVITY_ID" => $ID,
					"PARENTS" => (!empty($liveFeedFields['PARENTS']) ? $liveFeedFields['PARENTS'] : array())
				));
				$bConvert = true;
			}
		}
		...
		if (!$bConvert)
		{
			if (
				$arTask
				&& !empty($arTask['GROUP_ID'])
			)
			{
				$arSite = array();
				$rsGroupSite = CSocNetGroup::GetSite(intval($arTask['GROUP_ID']));
				if ($rsGroupSite)
				{
		...
		else
		{
			if ($arTask)
			{
				...
				{
					$perm = CSocNetFeaturesPerms::GetOperationPerm(SONET_ENTITY_GROUP, $arTask['GROUP_ID'], "tasks", "view_all");
			...
			)
			{
				...
				if ($type)
				{
					...
					if($bHasPermissions)
					{
						$arMessageFields["NOTIFY_MESSAGE"] = GetMessage("CRM_ACTIVITY_".$type."_RESPONSIBLE_IM_NOTIFY", Array("#title#" => '<a href="'.$url.'">'.htmlspecialcharsbx($arFields['SUBJECT']).'</a>'));
						$arMessageFields["NOTIFY_MESSAGE_OUT"] = GetMessage("CRM_ACTIVITY_".$type."_RESPONSIBLE_IM_NOTIFY", Array("#title#" => htmlspecialcharsbx($arFields['SUBJECT'])))." (".$serverName.$url.")";
					}
					else
					{
