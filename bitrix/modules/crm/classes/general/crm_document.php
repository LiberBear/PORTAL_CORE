...
class CCrmDocument
{
	...
	static public function UnlockDocument($documentId, $workflowId)
	{
		...
		$entityID = (int)$arDocumentID['ID'];
		$entityType = $DB->ForSQL($arDocumentID['TYPE']);
		$lockedBy = $DB->ForSQL($workflowId, 32);

		$strSql = "SELECT * FROM b_crm_entity_lock WHERE ENTITY_ID = {$entityID} AND ENTITY_TYPE = '{$entityType}'";
		$z = $DB->Query($strSql, false, 'FILE: '.__FILE__.'<br/>LINE: '.__LINE__);
		if($z->Fetch())
		{
			$z = $DB->Query(
				$lockedBy !== ''
					? "DELETE FROM b_crm_entity_lock WHERE ENTITY_ID = {$entityID} AND ENTITY_TYPE = '{$entityType}' AND LOCKED_BY = '{$lockedBy}'"
					: "DELETE FROM b_crm_entity_lock WHERE ENTITY_ID = {$entityID} AND ENTITY_TYPE = '{$entityType}'",
				false,
	...
	static public function IsDocumentLocked($documentId, $workflowId)
	{
		...
		$z = $DB->Query(
			"SELECT * FROM b_crm_entity_lock WHERE ENTITY_ID = {$entityID} AND ENTITY_TYPE = '{$entityType}' AND LOCKED_BY <> '{$lockedBy}'",
			false,
			'FILE: '.__FILE__.'<br/>LINE: '.__LINE__
		);
