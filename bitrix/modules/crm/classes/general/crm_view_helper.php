...
class CCrmViewHelper
{
	...
	private static $ENABLE_DEAL_STAGE_COLORS = false;
	...
	const PROCESS_COLOR = '#4C99DA';
	const SUCCESS_COLOR = '#96B833';
	const FAILURE_COLOR = '#F54819';
	...
	public static function RenderClientSummaryPanel($arParams, $arOptions = array())
	{
		...
		{
			echo '<div class="crm-detail-info-empty">',
				GetMessage($entityTypeName === CCrmOwnerType::CompanyName ? "CRM_CLIENT_SUMMARY_COMPANY_NOT_SPECIFIED" : "CRM_CLIENT_SUMMARY_CONTACT_NOT_SPECIFIED"),
				'</div>';
		}
	...
	public static function PrepareSipCallHtml($phone, $params = null)
	{
		...
		return '<span class="crm-tel-btn" onclick="if(typeof(window[\'BXIM\']) === \'undefined\') { window.alert(\''.GetMessageJS('CRM_SIP_NO_SUPPORTED').'\'); return; } BX.CrmSipManager.startCall({ number:\''.CUtil::JSEscape($phone).'\', enableInfoLoading: true }, { ENTITY_TYPE: \''.CUtil::JSEscape($entityType).'\', ENTITY_ID: \''.CUtil::JSEscape($entityID).'\' }, true, this);"></span>';
	}
	...
	public static function GetDealStageInfos()
	{
		return self::PrepareDealStages();
	}
	protected static function PrepareDealStages()
	{
		if(self::$DEAL_STAGES !== null)
		{
			return self::$DEAL_STAGES;
		}

		self::$DEAL_STAGES = CCrmDeal::GetStages();

		$scheme = Bitrix\Crm\Color\DealStageColorScheme::getCurrent();;
		if(!$scheme->isPersistent())
		{
			self::$ENABLE_DEAL_STAGE_COLORS = false;
		}
		else
		{
			self::$ENABLE_DEAL_STAGE_COLORS = true;
			foreach(self::$DEAL_STAGES as $ID => &$item)
			{
	...
	public static function RenderDealStageSettings()
	{
		...
		foreach(self::PrepareDealStages() as $stage)
		{
			$info = array(
				'id' => $stage['STATUS_ID'],
				'name' => $stage['NAME'],
	...
	public static function RenderDealStageControl($arParams)
	{
		...
		$arParams['INFOS'] = self::PrepareDealStages();
	...
	public static function RenderProgressControl($arParams)
	{
		...
		elseif($entityTypeName === $dealTypeName)
		{
			$enableCustomColors = self::$ENABLE_DEAL_STAGE_COLORS;
		}
		elseif($entityTypeName === $quoteTypeName)
		{
