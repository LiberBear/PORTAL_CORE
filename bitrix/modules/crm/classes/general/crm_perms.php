...
class CCrmPerms
{
	...
	static public function IsAccessEnabled($userPerms = null)
	{
		...
		return !$userPerms->HavePerm('LEAD', self::PERM_NONE)
			...
			|| !$userPerms->HavePerm('INVOICE', self::PERM_NONE);
	}
	...
	static public function BuildSql($enityType, $sAliasPrefix, $mPermType, $arOptions = array())
	{
		...
		foreach($permissionSets as &$permissionSet)
		{
			...
			else
			{
				...
				{
					$subQueries[] = "SELECT {$sAliasPrefix}P2.ENTITY_ID FROM b_crm_entity_perms {$sAliasPrefix}P2 WHERE {$sAliasPrefix}P2.ENTITY = '{$enityType}' AND {$scopeSql}";
				}
