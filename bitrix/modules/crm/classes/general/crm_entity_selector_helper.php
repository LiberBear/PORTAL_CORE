...
class CCrmEntitySelectorHelper
{
	public static function PrepareEntityInfo($entityTypeName, $entityID, $options = array())
	{
		...
		$bEntityEditorFormat = (
			is_array($options) && isset($options['ENTITY_EDITOR_FORMAT'])
			&& ($options['ENTITY_EDITOR_FORMAT'] === true || strtoupper($options['ENTITY_EDITOR_FORMAT']) === 'Y')
		);
		$bEntityPrefixEnabled = (
			is_array($options) && isset($options['ENTITY_PREFIX_ENABLED'])
		...
		if($entityTypeName === 'CONTACT')
		{
			...
			if($arRes = $obRes->Fetch())
			{
				...
				$result[$descKey] = empty($arRes['COMPANY_TITLE'])? '': $arRes['COMPANY_TITLE'];

				$result[$imageKey] = isset($arFiles[$photoID]['src']) ? $arFiles[$photoID]['src'] : '';
		...
		elseif($entityTypeName === 'COMPANY')
		{
			...
			if($arRes = $obRes->Fetch())
			{
				...
				$result[$imageKey] = isset($arFiles[$logoID]['src']) ? $arFiles[$logoID]['src'] : '';
