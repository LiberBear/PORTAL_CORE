...
class CAllCrmInvoice
{
	...
	public static function QuickRecalculate($arFields, $siteId = SITE_ID)
	{
		...
		$arOptions = array('LOCATION_IN_CODES' => true); // let DoCalculateOrder know we send location in CODEs
		$arErrors = array();
		$arWarnings = array();
	...
	public function Add($arFields, &$arRecalculated = false, $siteId = SITE_ID, $options = array())
	{
		...
		if ($tmpOrderId !== 0 && !isset($arFields['PRODUCT_ROWS']) && !isset($arFields['INVOICE_PROPERTIES']))
		{
			...
			$orderID = $order->Update($tmpOrderId, $arFields);
		...
		if (intval($orderID) > 0)
		{
			if (!empty($orderStatus) || $paidStateCanceled)
			{
				// set status
				$this->SetStatus($orderID, empty($orderStatus) ? 'P' : $orderStatus);
			}
		...
		if ($result)
		{
			...
			if ($result)
			{
				...
				if ($result)
				{
					...
					if (count($arUpdate) > 0)
					{
						self::Update($ID, $arUpdate, array('REGISTER_STATISTICS' => false));
					}
					unset($arUpdate);
				}
	...
	*/
	...
	public static function PrepareSalePaymentData(array &$arOrder)
	{
