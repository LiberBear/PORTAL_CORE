...
class CAllCrmDeal
{
	...
	public function Add(array &$arFields, $bUpdateSearch = true, $options = array())
	{
		...
		else
		{
			...
			$enableCloseDateSync = \Bitrix\Crm\Settings\DealSettings::getCurrent()->isCloseDateSyncEnabled();
	...
	public static function GetSemanticID($stageID)
	{
		if($stageID === 'WON')
		{
		...
		return (self::GetStageSort($stageID) > self::GetFinalStageSort())
	...
	public static function RebuildStatistics(array $IDs, array $options = null)
	{
		$dbResult = self::GetListEx(
			...
			false,
			array(
				'ID', 'DATE_CREATE', 'DATE_MODIFY', 'STAGE_ID',
		...
		while($fields = $dbResult->Fetch())
		{
			$ID = (int)$fields['ID'];
			//--> History
			if($enableHistory && ($forced || !Bitrix\Crm\History\DealStageHistoryEntry::isRegistered($ID)))
			{
				...
				{
					Bitrix\Crm\History\DealStageHistoryEntry::register(
						$ID,
						$fields,
			...
			//--> Statistics
			if($enableSumStatistics && ($forced || !Bitrix\Crm\Statistics\DealSumStatisticEntry::isRegistered($ID)))
			{
				Bitrix\Crm\Statistics\DealSumStatisticEntry::register($ID, $fields, array('FORCED' => $forced));
			}

			if($enableInvoiceStatistics && ($forced || !Bitrix\Crm\Statistics\DealInvoiceStatisticEntry::isRegistered($ID)))
			{
