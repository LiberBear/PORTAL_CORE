...
class CCrmOwnerType
{
	...
	const Quote = 7;
	const Requisite = 8;
	const System = 9; // refresh FirstOwnerType and LastOwnerType constants
	const FirstOwnerType = 1;
	const LastOwnerType = 9;
	...
	{
		return array(self::ContactName, self::CompanyName, self::LeadName, self::DealName, self::InvoiceName,
			self::ActivityName, self::QuoteName, self::Requisite);
	}
	...
	public static function GetAllCategoryCaptions()
	{
		if(!self::$ALL_CATEGORY_CAPTION[LANGUAGE_ID])
		{
			...
			self::$ALL_CATEGORY_CAPTION[LANGUAGE_ID] = array(
				...
				self::Quote => GetMessage('CRM_OWNER_TYPE_REQUISITE_CATEGORY'),
			);
		}
