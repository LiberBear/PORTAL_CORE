...
if($entityID <= 0)
{
	...
	if($contactID > 0)
	{
		$dbContact = CCrmContact::GetListEx(
			array(), array('=ID' => $contactID), false, false, array('HONORIFIC', 'NAME', 'LAST_NAME', 'SECOND_NAME', 'COMPANY_ID', 'COMPANY_TITLE')
		);
		...
		if(is_array($contact))
		{
			...
			$arFields['~CONTACT_HONORIFIC'] = isset($contact['HONORIFIC']) ? $contact['HONORIFIC'] : '';
			$arFields['CONTACT_HONORIFIC'] = htmlspecialcharsbx($arFields['~CONTACT_HONORIFIC']);
