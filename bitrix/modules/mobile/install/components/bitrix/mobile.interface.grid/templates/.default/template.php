...
{
?>
	<div class="mobile-grid-field-folder-list">
	<?foreach ($arResult["SECTIONS"] as $item):?>
		<div class="mobile-grid-field-folder" <?if (isset($item["ONCLICK"])):?>onclick="<?=$item["ONCLICK"] ?>"<?endif?>>
			...
			<div class="mobile-grid-field-item-folder"><?=htmlspecialcharsbx($item['TITLE'])?></div>
		</div>
	<?endforeach?>
	</div>
<?
				...
				<div class="mobile-grid-item" data-role="mobile-grid-item" data-id="<?=$item["DATA_ID"]?>">
					<div class="mobile-grid-field" <?if (isset($item["ONCLICK"])):?>onclick="<?=$item["ONCLICK"] ?>"<?endif?>>
						<?if (isset($item["ICON_HTML"])):?>
							<?=$item["ICON_HTML"]?>
						<?endif?>
						<span class="mobile-grid-field-lead-title"><span class="mobile-grid-field-lead-title-arrow"></span><?=$item["TITLE"]?></span>
					</div>
					...
					if (is_array($arResult["FIELDS"]) && !empty($arResult["FIELDS"]))
					{
						foreach ($arResult["FIELDS"] as $key => $field)
						{
							...
							switch ($field["type"])
							{
								case "PHONE":
									?>
									<div class="mobile-grid-field mobile-grid-field-phone">
										...
										<a class="mobile-grid-field-data" href="javascript:void();" onclick="BX.MobileTools.phoneTo('<?=$item["FIELDS"][$field["id"]]?>')"><?=$item["FIELDS"][$field["id"]]?></a>
										<span class="mobile-grid-field-textarea-title"><?=$field["name"]?></span>
									</div>
									<?
									break;
								case "EMAIL":
									?>
									<div class="mobile-grid-field mobile-grid-field-mail">
										...
										<a class="mobile-grid-field-data" href="mailto:<?=$item["FIELDS"][$field["id"]]?>"><?=$item["FIELDS"][$field["id"]]?></a>
										<span class="mobile-grid-field-textarea-title"><?=$field["name"]?></span>
									</div>
									<?
									break;
...
<script>
	...
	<?if ($arParams["SHOW_SEARCH"] == "Y"):?>
		BX.Mobile.Grid.searchInit(<?=CUtil::PhpToJSObject($arJsParams)?>);
	<?endif?>
</script>