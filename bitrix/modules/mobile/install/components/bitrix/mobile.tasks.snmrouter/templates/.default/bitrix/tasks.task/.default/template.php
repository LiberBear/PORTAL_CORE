...
{
	echo $arResult["TEMPLATE_DATA"]["ERROR"];
	return;
}
			...
			array(
				...
				"fields" => array(
					...
					array(
						...
						"value" => "<div id=\"checkList".$task["ID"]."Container\">".(($checkList = function($list, &$ids)
							{
								$result = "";
								$ids = array();
...
/*					array( // TODO make tasks relations
						"id" => "mobile-grid-field-tasklinks",
						"type" => "custom",
						"name" => "Tasks relations",
						"class" => "mobile-grid-field-tasklinks",
						"value" => "<ol class=\"mobile-grid-field-tasklinks\">".
							($links = function($links){
								...
								<li>
									<div class="mobile-grid-field">
										<span class="mobile-grid-field-title">task</span>
										<div class="mobile-grid-field-label">
											<i class="mobile-grid-menu"></i>
											<span>task name</span>
										</div>
									</div>
									<div class="mobile-grid-field">
										<span class="mobile-grid-field-title">action</span>
										<div class="mobile-grid-field-select">
											<a href="#">begin</a>
										</div>
									</div>
									<div class="mobile-grid-field">
										<span class="mobile-grid-field-title">when</span>
										<div class="mobile-grid-field-select">
											<a href="#">ends</a>
										</div>
									</div>
									<div class="mobile-grid-field">
										<span class="mobile-grid-field-title">delay</span>
										<div class="mobile-grid-field-label">
											5 days
										</div>
									</div>
...
HTML;
				...
				"<a href=\"#\" class=\"mobile-grid-button\">".GetMessage("MB_TASKS_TASK_ADD")."</a>"
					),*/
					array(
						"type" => "select-group",
					...
					array(
						...
						"value" =>
							"<div class=\"mobile-grid-field-custom-item\" id=\"parentId".$task["ID"]."Container\">".
								"<span>".$task["PARENT_TITLE"]."</span>".
							"</div>"
					) : null),
/*					((!($task["ID"] > 0) && ($can["FAVORITE.ADD"] || $can["FAVORITE.DELETE"])) ?
					array(
						"type" => "checkbox",
						"id" => "data[ADD_TO_FAVORITE]",
						"params" => array(
							"class" => "mobile-grid-field-favorites"
						),
						"name" => GetMessage("MB_TASKS_BASE_SETTINGS_FAVORITES"),
						"items" => array("Y" => ($task["FAVORITE"] == "Y" ? GetMessage("TASKS_FAVORITES_1") : GetMessage("TASKS_FAVORITES_0"))),
						"value" => array($task["FAVORITE"])
					) : null )
*/
				)
			)
...
BX.ready(function(){
	app.hidePopupLoader();
	BX.message({ PAGE_TITLE : '<?=($task["ID"] > 0 ? GetMessageJS('MB_TASKS_TASK_TITLE_EDIT') : GetMessageJS('MB_TASKS_TASK_TITLE_CREATE'))?>'} );
