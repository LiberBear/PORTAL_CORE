...
if (in_array("DEADLINE", $fields))
	array_splice($fields, array_search("DEADLINE", $fields)+1, 0, "EXPIRED");
...
foreach ($arResult["TASKS"] as $task)
{
	...
	if ($task['META:ALLOWED_ACTIONS']['ACTION_EDIT'])
	{
		$task["PRIORITY"] = "<label for=\"bx-task-priority-".$task["ID"]."\" class=\"mobile-grid-field-priority\">".
				...
				"<span id=\"bx-task-priority-".$task["ID"]."-span\">".($task["~PRIORITY"] == CTasks::PRIORITY_HIGH ? GetMessage("TASKS_PRIORITY_2") : GetMessage("TASKS_PRIORITY_0"))."</span>".
			"</label>";
	}

	$task["MARK"] = '<span id="bx-task-mark-'.$task["ID"].'">' . GetMessage("TASKS_MARK_".$task["MARK"])."</span>";
	...
	{
		$task["TIME_ESTIMATE"] = ' / ' . sprintf(
			'%02d:%02d',
			floor($arResult['TASK']['TIME_ESTIMATE']  / 3600),	// hours
			floor($arResult['TASK']['TIME_ESTIMATE'] / 60) % 60	// minutes
		);
	}
	$task["ALLOW_TIME_TRACKING"] = ($task["ALLOW_TIME_TRACKING"] == "Y" ? GetMessage("TASK_ALLOWED") : GetMessage("TASK_DISALLOWED"));
	...
	if ($task["META:ALLOWED_ACTIONS"]["ACTION_ADD_FAVORITE"] || $task["META:ALLOWED_ACTIONS"]["ACTION_DELETE_FAVORITE"])
		$task["FAVORITES"] = '<label for="bx-task-favorites-'.$task["ID"].'" class="mobile-grid-field-favorites">'.
			'<input id="bx-task-favorites-'.$task["ID"].'" type="checkbox"'.($task["META:ALLOWED_ACTIONS"]["ACTION_DELETE_FAVORITE"] ? " checked=\"checked\"" : "") .' />'.
			'<span id="bx-task-favorites-'.$task["ID"].'-span">'.($task["META:ALLOWED_ACTIONS"]["ACTION_DELETE_FAVORITE"] ? GetMessage("TASKS_FAVORITES_1") : GetMessage("TASKS_FAVORITES_0")).'</span>'.
		'</label>';
