				...
				)
				{
					...
					setTimeout(function() {
						if (!oMSL.bCounterReceived)
						{
							...
							BXMobileApp.onCustomEvent('onGetLFCounter', obTmp);

							setTimeout(function() {
								if (obTmp.value >= 0)
								{
									oMSL.bCounterReceived = true;
									oMSL.changeCounter(parseInt(obTmp.value), obTmp.zeroCounter);
								}
	...
	BMAjaxWrapper.Wrap({
		...
		callback: function(data)
		{
			...
			)
			{
				...
				{
					app.onCustomEvent('onClearLFCounter', [data.COUNTER_TO_CLEAR]);
				}
