...
?><div style="width: 542px; min-height: 200px; padding: 5px; overflow-y: auto; margin: 5px;">
			...
			elseif($strAction == "self")
			{
				if (\Bitrix\Main\Loader::includeModule("socialservices"))
				{
					\Bitrix\Socialservices\Network::setRegisterSettings(array(
						...
						"REGISTER_TEXT" => $_POST["allow_register_text"] && $_POST["allow_register_text"] != GetMessage("BX24_INVITE_DIALOG_REGISTER_TEXT_PLACEHOLDER") ? $_POST["allow_register_text"] : "",
	...
	?>
		<div class="intranet-tabs-box" id="intranet-dialog-tabs"><?
			...
			<span class="intranet-tab<?=(IsModuleInstalled("bitrix24")  ? " intranet-tab-selected" : "")?>" id="intranet-dialog-tab-self" data-action="self">
				<?=GetMessage('BX24_INVITE_DIALOG_TAB_SELF_TITLE')?>
			</span>
			<?endif?>

			<span class="intranet-tab<?=(!IsModuleInstalled("bitrix24")  ? " intranet-tab-selected" : "")?>" id="intranet-dialog-tab-invite" data-action="invite">
			...
			?>
				<div class="intranet-tab-content<?=(IsModuleInstalled("bitrix24") ? " intranet-tab-content-selected" : "")?>" id="intranet-dialog-tab-content-self" data-user-type="employee">
					<form method="POST" action="<?echo BX_ROOT."/tools/intranet_invite_dialog.php"?>" id="SELF_DIALOG_FORM">
						<div style="margin-right: 10px;">
							<div class = "invite-dialog-inv-text-bold">
								...
								<label for="allow_register"><?=GetMessage("BX24_INVITE_DIALOG_REGISTER_ALLOW")?></label>
							</div>
							<div class = "invite-dialog-inv-text-bold" style="margin-left: 25px">
								<?=GetMessage("BX24_INVITE_DIALOG_REGISTER_TEXT")?>
							</div>
							...
							<div <?if ($registerSettings["REGISTER"] != "Y"):?>style="display: none"<?endif?> id="intranet-dialog-tab-content-self-block">
								...
								<div id="intranet-dialog-tab-content-self-hidden-block" style="display: none; margin-left:27px">
									<div class = "invite-dialog-inv-text-bold">
										<p><b><?=GetMessage("BX24_INVITE_DIALOG_REGISTER_TYPE")?></b></p>
										<input
											type="radio"
										...
										/>
										<label for="allow_register_confirm_y"><?=GetMessage("BX24_INVITE_DIALOG_REGISTER_TYPE_OPEN")?></label>
										...
										<input
											...
											<?if (!$isUserAdmin) echo "disabled";?>
										>
										<label for="allow_register_confirm_n"><?=GetMessage("BX24_INVITE_DIALOG_REGISTER_TYPE_CLOSE")?></label>
									...
									<div class = "invite-dialog-inv-text-bold">
										...
										<textarea <?if (!$isUserAdmin) echo "disabled";?> name="allow_register_text" placeholder="<?=GetMessage("BX24_INVITE_DIALOG_REGISTER_TEXT_PLACEHOLDER")?>" class="invite-dialog-inv-form-textarea invite-dialog-inv-form-textarea-active"><?
											?><?=$registerSettings["REGISTER_TEXT"] ? htmlspecialcharsbx($registerSettings["REGISTER_TEXT"]) : GetMessage("BX24_INVITE_DIALOG_REGISTER_TEXT_PLACEHOLDER")?><?
										?></textarea>
									</div>
								</div>
							</div>
			...
			?>
				<div class="intranet-tab-content<?=(!IsModuleInstalled("bitrix24") ? " intranet-tab-content-selected" : "")?>" id="intranet-dialog-tab-content-invite" data-user-type="employee"><?
