...
class CIntranetInviteDialog
{
	...
	function AddNewUser($SITE_ID, $arFields, &$strError)
	{
		...
		if (!$strError)
		{
			...
			if ($iDepartmentId > 0)
			{
				$arUser["UF_DEPARTMENT"] = array($iDepartmentId);
			}
	...
	public static function RegisterUser($userData, $SITE_ID = SITE_ID)
	{
		...
		if (intval($userData["UF_DEPARTMENT"]) > 0)
		{
			$arUser["UF_DEPARTMENT"] = array($userData["UF_DEPARTMENT"]);
		}
	...
	public function TransferEmailUser($userId, $arParams = array())
	{
		global $APPLICATION, $USER;
