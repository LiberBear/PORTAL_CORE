...
final class Comment extends \Bitrix\Tasks\Integration\Forum\Comment
{
	 ...
	 * @param $messageText
	 * @param bool $authorId
	 * @param array $files Attachment temporal IDs (i.e. 'n17') of files that were uploaded to disk
	 * @return array|bool
	 *
	 * @access private
	 */
	public static function add($taskId, array $data)
	{
		...
		// currenly, there is the only way to pass file attachments to $feed->add() :(
		if(!array_key_exists('UF_FORUM_MESSAGE_DOC', $data))
		{
			$GLOBALS["UF_FORUM_MESSAGE_DOC"] = $data['UF_FORUM_MESSAGE_DOC'];
			unset($data['UF_FORUM_MESSAGE_DOC']);
		}
...
//					"URL" => str_replace("?IFRAME=Y", "", str_replace("&IFRAME=Y", "", str_replace("IFRAME=Y&", "", $strURL))),
	...
	/**
	 * Event callback for comment update\delete. Fires on forum::OnAfterCommentUpdate
	 *
	 * This is not a part of public API.
	 * This function is for internal use only.
	 *
