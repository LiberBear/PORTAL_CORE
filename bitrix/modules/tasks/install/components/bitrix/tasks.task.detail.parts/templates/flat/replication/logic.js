	...
	var timePicker = BX.Tasks.Util.Widget.extend({
		...
		methods: {
			construct: function()
			{
				...
				this.setTime(this.parseTime(this.option('value')));

				// when main 16.0.12 released, remove this:
				var input = BX(this.option('inputId'));
				var oldVal = input.value;
				setInterval(BX.delegate(function(){
					if(input.value != oldVal)
					{
						oldVal = input.value;
						this.onTimeChange(input);
					}
				}, this) , 500);
				// and uncomment this:
			...
			parseTime: function(value)
			{
				...
				var found = time.match(new RegExp('^(\\d{1,2}):(\\d{2})\\s*(am|pm)?', 'i'));

				var h = parseInt(found[1]);
				var m = parseInt(found[2]);
				var hasAmPm = !!found[3];
				var pm = (hasAmPm && found[3].toLowerCase() == 'pm');
