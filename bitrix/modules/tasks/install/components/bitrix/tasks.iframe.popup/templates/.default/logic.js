...
BX.mergeEx(BX.Tasks.Component.IframePopup.prototype, {
	...
	open: function(action, taskId, parameters)
	{
		...
		this.getIframe().src = this.getPath(action, taskId, parameters.urlParams);
	},
	close: function()
	{
	...
	bindEvents: function()
	{
		...
		// see "wrap" template
		BX.addCustomEvent(window, 'tasksIframeLoad', BX.delegate(this.onContentLoaded, this));
		BX.addCustomEvent(window, 'tasksIframeUnload', BX.delegate(this.onContentUnLoaded, this));
	},
	getIframe: function()
	{
	...
	{
		var innerDoc = (this.getIframe().contentDocument) ? this.getIframe().contentDocument : this.getIframe().contentWindow.document;
	...
	getIframeContainer: function()
	{
		if (this.ctrls.wrap === null)
		{
			this.ctrls.wrap = this.ctrls.wrap = BX.create('div', {
				props: { className: 'loading fixedHeight'},
	...
	{
		return this.getContentDocument().getElementById("tasks-content-outer"); // see tasks.iframe.popup "wrap" template for "tasks-content-outer" node
	},
	...
	onTaskGlobalEvent: function(eventType, params)
	{
		if(BX.type.isNotEmptyString(eventType))
		{
			...
			{
				var cbName = this.vars.callbacks[cbAction];

				if(BX.type.isFunction(window[cbName]))
				{
					window[cbName].apply(window, args);
				}
			}
		}
	},
	...
	setCallbacks: function(callbacks)
	{
		if(BX.type.isPlainObject(callbacks))
		{
			...
			{
				if(callbacks[k] !== false && BX.type.isNotEmptyString(callbacks[k]))
				{
					this.vars.callbacks[k] = callbacks[k];
				}
			}
		}
	},
