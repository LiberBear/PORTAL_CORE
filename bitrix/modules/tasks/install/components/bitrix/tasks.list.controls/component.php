...
)
{
	...
	if(is_array($arResult['VIEW_STATE']['SPECIAL_PRESETS']))
	{
		...
		{
			$arResult['VIEW_HREFS']['SPECIAL_PRESETS'][$presetId] = $arParams["PATH_TO_TASKS"].'?'.(isset($_GET['VIEW']) ? 'VIEW='.intval($_GET['VIEW']).'&' : '').'F_CANCEL=Y&F_FILTER_SWITCH_PRESET='.$presetId.'&F_STATE[]=sC'.CTaskListState::encodeState(CTaskListState::VIEW_TASK_CATEGORY_ALL);
		}
	}
