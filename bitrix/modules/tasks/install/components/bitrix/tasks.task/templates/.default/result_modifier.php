...
if(false && is_array($arResult['AUX_DATA']['USER_FIELDS'][$ufCode]))
{
...
// user paths
$this->__component->tryParseStringParameter($arParams["PATH_TO_USER_TASKS"], COption::GetOptionString("tasks", "paths_task_user", null, SITE_ID));
$this->__component->tryParseStringParameter($arParams["PATH_TO_USER_TASKS_TASK"], COption::GetOptionString("tasks", "paths_task_user_action", null, SITE_ID));

// group paths
$this->__component->tryParseStringParameter($arParams["PATH_TO_GROUP_TASKS"], COption::GetOptionString("tasks", "paths_task_group", null, SITE_ID));
$this->__component->tryParseStringParameter($arParams["PATH_TO_GROUP_TASKS_TASK"], COption::GetOptionString("tasks", "paths_task_group_action", null, SITE_ID));

// template paths
$this->__component->tryParseStringParameter($arParams["PATH_TO_USER_TEMPLATES_TEMPLATE"], '');
$this->__component->tryParseStringParameter($arParams["PATH_TO_USER_TASKS_TEMPLATES"], '');
...
// Blocks
// if no user fields, hide block
$noUfs = true;
if(is_array($arResult['AUX_DATA']['USER_FIELDS']))
{
    foreach($arResult['AUX_DATA']['USER_FIELDS'] as $code => $info)
    {
        if($code != 'UF_TASK_WEBDAV_FILES' && $code != 'UF_CRM_TASK')
        {
            $noUfs = false;
            break;
        }
...
if($noUfs)
{
    unset($arResult['COMPONENT_DATA']['STATE']['BLOCKS']['USER_FIELDS']);
}
		...
		if(array_key_exists(TasksTaskFormState::O_CHOSEN, $state))
		{
			...
			else // "special" cases
			{
                ...
                if($block == 'USER_FIELDS')
                {
                    ...
                    {
                        if(strpos($field, 'UF_') == 0 && !empty($value))
                        {
                            $filled = true;
                            break;
                        }
