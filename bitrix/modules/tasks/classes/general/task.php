...
use \Bitrix\Tasks\Util\Scheduler;
...
class CTasks
{
	...
	public function Add($arFields, $arParams = array())
	{
		...
		if ($this->CheckFields($arFields, false, $effectiveUserId))
		{
			if($arParams['CLONE_DISK_FILE_ATTACHMENT'] === true || $arParams['CLONE_DISK_FILE_ATTACHMENT'] === 'Y')
			{
				...
				{
					$arFields['UF_TASK_WEBDAV_FILES'] = \Bitrix\Tasks\Integration\Disk::cloneFileAttachment($effectiveUserId, $arFields['UF_TASK_WEBDAV_FILES']);
				}
			}
					...
					if ($result)
					{
						...
						if($arParams['CORRECT_DATE_PLAN'] && $arParams['CORRECT_DATE_PLAN_DEPENDENT_TASKS'] && (isset($arFields["START_DATE_PLAN"]) || isset($arFields["END_DATE_PLAN"])))
						{
							...
							if(is_array($relatedTaskUpdate))
							{
								...
								$relatedUpdateParams['CORRECT_DATE_PLAN'] = false; // no self-correction
								$relatedUpdateParams['CORRECT_DATE_PLAN_DEPENDENT_TASKS'] = false; // no sub-correction

								foreach($relatedTaskUpdate as $relatedTaskId => $relatedTaskData)
								{
									$relatedTaskData['START_DATE_PLAN_STRUCT'] = 	$relatedTaskData['START_DATE_PLAN']->getTimeStruct(); // as struct
									$relatedTaskData['END_DATE_PLAN_STRUCT'] = 		$relatedTaskData['END_DATE_PLAN']->getTimeStruct(); // as struct

									$relatedTaskData['START_DATE_PLAN'] = 			$relatedTaskData['START_DATE_PLAN']->toString(); // as string, not object
									$relatedTaskData['END_DATE_PLAN'] = 			$relatedTaskData['END_DATE_PLAN']->toString(); // as string, not object
		...
		foreach ($arFilter as $key => $val)
		{
			...
			switch ($key)
			{
				...
				case "ONLY_ROOT_TASKS":
					if ($val == "Y")
					{
						$arSqlSearch[] = "(".$sAliasPrefix."T.PARENT_ID IS NULL OR NOT EXISTS (".CTasks::GetRootSubquery($arFilter, $bGetZombie, $sAliasPrefix)."))";
					}
					break;
	...
	/*
	private static function printTime($label, $stamp)
	{
		global $DB;

		if(is_numeric($stamp))
		{
			_dump_r($label.': '.date($DB->DateFormatToPHP(FORMAT_DATETIME), $stamp));
		}
		else
		{
			_dump_r($label.': '.$stamp);
	...
	*/
	 ...
	 */
	public static function RepeatTaskByTemplateId ($templateId)
	{
		...
		{
			$agentName = 'CTasks::RepeatTaskByTemplateId('.$templateId.');';

			if (intval($arTemplate['CREATED_BY']))
			{
				\Bitrix\Tasks\Util\User::setOccurAsId($arTemplate['CREATED_BY']); // not admin in logs, but template creator
			}
			...
			try
			{
				CTaskItem::addByTemplate($templateId, \Bitrix\Tasks\Util\User::getAdminId(), array(), array(
			...
			{
				// Task replication failed, log smth
			...
			//_dump_r('By '.$templateId);

			$nextPossibleTime = $agentTime;
			do
			{
				//static::printTime('Next possible', $nextPossibleTime);

				$nextTime = CTasks::getNextTime($arTemplate['REPLICATE_PARAMS'], $arTemplate, $nextPossibleTime);
				if(MakeTimeStamp($nextPossibleTime) == MakeTimeStamp($nextTime))
				{
					// possible endless loop, this agent must be stopped
					$exceptionHandler->writeToLog(new TasksException('getNextTime() loop detected for replication by template '.$templateId.' ('.$nextTime.' => '.$nextPossibleTime.')'));
				...
				//static::printTime('Calculated', $nextTime);

				$nextPossibleTime = $nextTime;
				// we can compare one user`s time only with another user`s time, we canna just take time() value
				$cTime = time() + \Bitrix\Tasks\Util\User::getTimeZoneOffsetCurrentUser();

				//static::printTime('Compare with', $cTime);
			}
			while($nextTime && MakeTimeStamp($nextTime) < $cTime);
	...
	public static function getNextTime($arParams, $template = false, $agentTime = false)
	{
		...
		$arParams = CTaskTemplates::parseReplicationParams($arParams);
		...
		$time = $arParams["TIME"];
		$timeAmount = \Bitrix\Tasks\UI::parseTimeAmount($time);
		...
		{
			$item = CTaskTemplates::getList(array(), array('ID' => $template), array(), array(), array('CREATED_BY', 'TPARAM_REPLICATION_COUNT'))->fetch();
			if($item)
			{
				$templateData = $item;
			}
		...
		{
			$timeZoneOffset = intval($arParams['TIMEZONE_OFFSET']);
		}
		else
		{
			$createdBy = intval($templateData['CREATED_BY']);
			$timeZoneOffset = \Bitrix\Tasks\Util\User::getTimeZoneOffset($createdBy);
		}

		// lets calculate time base
		$baseTime = time();
		if($agentTime) // agent were found and had legal next_time
		{
			$agentTime = MakeTimeStamp($agentTime);
			if($agentTime) // time parsed normally
			{
				// $agentTime is in current user`s time, but we want server time here
				if($currentUserId)
				{
					$agentTime -= \Bitrix\Tasks\Util\User::getTimeZoneOffset($currentUserId);
				}
		...
		{
			$startTime = MakeTimeStamp($arParams["START_DATE"]); // its in local time of timezone specified in TIMEZONE_OFFSET
			$startTime -= $timeZoneOffset;
		}

		// now strip time, make only dates
		$m = (int) date("n", $baseTime);
		$d = (int) date("j", $baseTime);
		$y = (int) date("Y", $baseTime);
		$baseDate = mktime(0, 0, 0, $m, $d, $y);

		// now strip time, make only dates
		$m = (int) date("n", $startTime);
		$d = (int) date("j", $startTime);
		$y = (int) date("Y", $startTime);
		$startDate = mktime(0, 0, 0, $m, $d, $y);

		// now get max of dates and add time
		$startDate = max($baseDate, $startDate);
		$startDate = date("Y-m-d ".$time, $startDate); // its a (creator or current) user`s time!

		if ($endDate)
		{
			$endDate = date("Y-m-d ".$time, MakeTimeStamp($endDate));
		}
		...
		switch ($type)
		{
			case "daily":
				...
				$date = strtotime($startDate." +".$num." days"); // user time

				if ($workDays)
				{
					$dateInst = \Bitrix\Tasks\Util\Type\DateTime::createFromTimestamp($date)->convertToGmt();
					$calendar = new \Bitrix\Tasks\Util\Calendar();

					$workTime = $calendar->getClosestWorkTime($dateInst);
					$workTime->stripTime();
					$workTime->addSecond($timeAmount);

					$date = $workTime->convertToLocalTime()->getTimestamp();
				}
		...
		if (isset($date) && $date)
		{
			...
			if(array_key_exists("REPEAT_TILL", $arParams) && $arParams['REPEAT_TILL'] != 'endless')
			{
				...
				{
					$proceed = !($endDate && $date > strtotime($endDate));
				}
			...
			if($proceed)
			{
				// here we got $date with offset specified in TIMEZONE_OFFSET
				// we need to make output in current user`s tz offset (if we have user) or in server`s (local) offset (otherwise)
				...
				if($currentUserId)
				{
					// to current user time
					$date += \Bitrix\Tasks\Util\User::getTimeZoneOffset($currentUserId);
				}
