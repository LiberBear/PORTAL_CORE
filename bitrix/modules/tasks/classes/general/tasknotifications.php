...
use \Bitrix\Tasks\Notification\ThrottleTable;
...
class CTaskNotifications
{
	...
	public static function cropMessage($template, array $replaces = array(), $length = false)
	{
		...
		else
		{
			...
			// todo: make more clever algorythm here
	...
	public static function getNotificationPath($arUser, $taskID, $bUseServerName = true, $arSites = array())
	{
		...
		if (is_array($arTask) && !empty($arTask))
		{
			...
			$bExtranet = 	CModule::IncludeModule('extranet') && !self::checkUserIsIntranet($arUser["ID"]);
			// task is in a group
	...
	private static function getEffectiveUserId(array $arFields = array(), array $arTask = array(), $bSpawnedByAgent = false, array $parameters = array())
	{
		...
		{
			if(is_object($USER) && method_exists($USER, 'getId') && $bSpawnedByAgent !== true && $bSpawnedByAgent !== 'Y')
			{
				$effectiveUserId = (int) $USER->getId();
			}
			else
	 ...
	 * @deprecated
	 */
	function GetRecipientsIDs($arFields, $bExcludeCurrent = true, $bExcludeAdditionalRecipients = false, $currentUserId = false)
	{
		global $USER;
	...
	private static function notifyByMail(array $message, array $site)
	{
		...
		if (!\Bitrix\Tasks\Integration\Mail::checkModuleInstalled())
		{
			return false;
		}
		...
		foreach ($receiversData as $userId => $arUser)
		{
			...
			{
				$e = array(
					"=Reply-To" => $users[$authorId]['NAME_FORMATTED'].' <'.$replyTo.'>',
					"=Message-Id" => $threadMessageId,
					"=In-Reply-To" => \Bitrix\Tasks\Integration\Mail::formatThreadId('TASK_'.$taskId, $siteId),
					"EMAIL_FROM" => $users[$authorId]['NAME_FORMATTED'].' <'.\Bitrix\Tasks\Integration\Mail::getDefaultEmailFrom($siteId).'>',
