...
final class CTaskItem implements CTaskItemInterface, ArrayAccess
{
	...
	protected function getChildTemplateData($templateId)
	{
		...
		// get check lists
		$res = \Bitrix\Tasks\Template\CheckListTable::getListByTemplateDependency($templateId, array(
	...
	{
		if($parameters['CHECK_BY_DATA_FETCH'])
		{
			try
			{
	 ...
	 * @throws TasksException
	 */
	public function startWatch($userId = 0)
	{
		// Force reload cache
		$this->markCacheAsDirty();
		$arTask = $this->getData($bEscaped = false);
	...
	public function getDurationType()
	{
		...
		if((string) $this->arTaskData['DURATION_TYPE'] == '' || !in_array($this->arTaskData['DURATION_TYPE'], array(
			static::TIME_UNIT_TYPE_DAY,
			static::TIME_UNIT_TYPE_HOUR,
			static::TIME_UNIT_TYPE_MINUTE
		)))
		{
			return static::TIME_UNIT_TYPE_HOUR;
		}
