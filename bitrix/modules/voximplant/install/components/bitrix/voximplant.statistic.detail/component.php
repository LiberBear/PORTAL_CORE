...
$navId = $arParams['NAV_ID']? intval($arParams['NAV_ID']): 1;
$arResult["GRID_ID"] = "voximpant_statistic_detail_".$navId;
...
$page = ($_GET['PAGEN_'.$navId]) ? $_GET['PAGEN_'.$navId] : 1;
...
if (isset($_GET['VOTE']))
{
	$filterByVote = intval($_GET['VOTE']);
	$filter["CALL_VOTE"] = $filterByVote;

}

$filter["CALL_CATEGORY"] = 'external';
if (isset($_GET['INTERNAL']) && $_GET['INTERNAL'] == 'Y')
{
	unset($filter["CALL_CATEGORY"]);
}
...
foreach($arResult["ELEMENTS_ROWS"] as $key => $arRow)
{
	if ($arRow["data"]["PORTAL_USER_ID"])
	{
		...
		$userHtml.= "></span><a href='?USER_ID=".$arRow["data"]["PORTAL_USER_ID"].($filterByCode? '&CODE='.$filterByCode :'').($filterByVote !== ''? '&VOTE='.$filterByVote :'')."'>".$arUsers[$arRow["data"]["PORTAL_USER_ID"]]["FIO"]."</a>";
	}
	else
	{
	...
	$arResult["ELEMENTS_ROWS"][$key]["columns"]["CALL_FAILED_REASON"] = '<a href="?CODE='.$arRow["data"]["CALL_FAILED_CODE"].($filterByUser? '&USER_ID='.$filterByUser :'').($filterByVote !== ''? '&VOTE='.$filterByVote :'').'">'.$arRow["data"]["CALL_FAILED_REASON"].'</a>';
	...
	if (strlen($arRow["data"]["CALL_RECORD_HREF"]) > 0)
	{
		...
		if(isset($arRow["data"]["CALL_RECORD_DOWNLOAD_URL"]))
			$recordHtml .= '<a href="'.$arRow["data"]["CALL_RECORD_DOWNLOAD_URL"].'" target="_blank" class="tel-player-download"></a>';
		else
	...
	if (intval($arRow["data"]["CALL_VOTE"]) == 0)
	{
		$arResult["ELEMENTS_ROWS"][$key]["columns"]["CALL_VOTE"] = '-';
	}
	else
	{
		$arResult["ELEMENTS_ROWS"][$key]["columns"]["CALL_VOTE"] = '<a href="?VOTE='.$arRow["data"]["CALL_VOTE"].($filterByCode? '&CODE='.$filterByCode :'').($filterByUser? '&USER_ID='.$filterByUser :'').'">'.($arRow["data"]["CALL_VOTE"]).'</a>';
	}
}
