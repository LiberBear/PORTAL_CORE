...
class CVoxImplantSip
{
	...
	public function PrepareFiedls($fields, $type = self::CHECK_ADD)
	{
		$result = Array();
		$errors = Array();
	...
	private function UpdateSipRegistration($regId, $server, $login, $password = '')
	{
		$viHttp = new CVoxImplantHttp();
		$result = $viHttp->UpdateSipRegistration($regId, $server, $login, $password);
		if (!$result)
			return false;
