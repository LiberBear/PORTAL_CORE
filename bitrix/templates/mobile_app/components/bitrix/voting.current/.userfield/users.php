...
$arVoteList = array(
	...
	"items" => array(),
	"StatusPage" => "done",
	"url" => (!empty($_REQUEST["URL_TEMPLATE"]) ? urldecode($_REQUEST["URL_TEMPLATE"]) : '/company/personal/user/#USER_ID#/'));
$arUsers = array();
if ($_REQUEST["answer_id"] > 0/* && check_bitrix_sessid()*/)
{
	...
	$res = (($arParams["CACHE_TIME"] > 0 && $cache->InitCache($arParams["CACHE_TIME"], $cache_id, $cache_path)) ?
		$cache->GetVars() : array());
