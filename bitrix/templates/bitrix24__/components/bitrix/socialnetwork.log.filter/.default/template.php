		...
		if ($arParams["USE_SONET_GROUPS"] != "N")
		{
			?><div class="filter-field">
				<div class="filter-field-title-block">
					<span id="filter-dest-group-tab" class="filter-field-title-tab<?=(empty($arResult["ToUser"]) ? "" : " webform-field-action-link")?>" onclick="onFilterDestChangeTab('group');"><?=GetMessage("SONET_C30_T_FILTER_GROUP");?></span><?
					?><span class="filter-field-title-tab">&nbsp;/&nbsp;</span><?
					?><span id="filter-dest-user-tab" class="filter-field-title-tab<?=(empty($arResult["ToUser"]) ? " webform-field-action-link" : "")?>" onclick="onFilterDestChangeTab('user');"><?=GetMessage("SONET_C30_T_FILTER_USER");?></span>
				</div>
