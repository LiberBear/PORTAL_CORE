	...
	BX.CrmQuickPanelView.prototype =
	{
		...
		saveConfig: function(forAllUsers, callback)
		{
			...
			for(var k in this._sections)
			{
				...
				BX.userOptions.save('crm.entity.quickpanelview', this._id, k, config[k], forAllUsers);
			}

			BX.userOptions.save('crm.entity.quickpanelview', this._id, 'enabled', config.enabled, forAllUsers);
			BX.userOptions.save('crm.entity.quickpanelview', this._id, 'expanded', config.expanded, forAllUsers);
			BX.userOptions.save('crm.entity.quickpanelview', this._id, 'fixed', config.fixed, forAllUsers);
		...
		adjust: function()
		{
			...
			if (BX.CrmQuickPanelView.getNodeRect(this._placeholder).top <= heightOffset)
			{
				...
				{
					var r = BX.CrmQuickPanelView.getNodeRect(this._wrapper);
...
if(typeof(BX.CrmQuickPanelMultiField) === "undefined")
{
	...
	BX.CrmQuickPanelMultiField.prototype.layout = function()
	{
		...
		var wrapper = BX.create("SPAN", { attrs: { className: "crm-detail-info-item-text" } });
		this._container.appendChild(wrapper);
		...
		if(type === "PHONE" && BX.type.isNotEmptyString(firstItem["sipCallHtml"]))
		{
			...
			BX.addClass(wrapper, "crm-detail-info-item-handset");
		}

		if(items.length > 1)
		{
			BX.addClass(wrapper, "crm-detail-info-item-list");
			this._openListButton = BX.create(
				"SPAN",
				{
					attrs: { className: "crm-item-tel-list" }
				}
			);
	...
	BX.CrmQuickPanelMultiField.adjust = function()
	{
		...
		var elements = cssQuery(".crm-detail-info-item-text", this._wrapper);
...
if(typeof(BX.CrmQuickPanelClientInfo) === "undefined")
{
	...
	BX.CrmQuickPanelClientInfo.prototype.layout = function()
	{
		...
		var linkClassName = "crm-detail-info-resp";
		...
		var imageContainer = BX.create("DIV", { attrs: { className: entityTypeName === "COMPANY" && isNotEmpty ? "crm-lead-header-company-img" : "crm-detail-info-resp-img" } });
		...
		else
		{
			this._link.appendChild(
				BX.create(
					"DIV",
					{
						attrs: { className: "crm-detail-info-empty" },
...
if(typeof(BX.CrmQuickPanelAddress) === "undefined")
{
	...
	BX.CrmQuickPanelAddress.prototype.doInitialize = function()
	{
		...
		this._openPopupButton = BX.findChild(this._container, { tagName: "SPAN", className: "crm-item-tel-list" }, true, false);
	...
	BX.CrmQuickPanelAddress.prototype.layout = function()
	{
		...
		{
			wrapper = BX.create("SPAN", { attrs: { className: "crm-detail-info-item-text" } });
			this._container.appendChild(wrapper);
