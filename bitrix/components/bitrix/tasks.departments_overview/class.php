...
class CTasksDepartmentsOverviewComponent extends CBitrixComponent
{
	public function executeComponent()
	{
		...
		foreach ($arDepartmentsData as $departmentId => $departmentName)
		{
			...
			$rsUsers = CTaskIntranetTools::getDepartmentsUsers(
				array($departmentId),
				array(
			...
			if ($departmentHead > 0)
			{
				$arImmediateEmployees = CTaskIntranetTools::getImmediateEmployees(
					$departmentHead,
	...
	private function processParams()
	{
		...
		if (isset($USER) && is_object($USER))
			$this->arResult['LOGGED_IN_USER'] = (int) $USER->getId();
		else
		...
		else
		{
			...
			$startFromDepartmentsDraft = array_unique(
				array_filter(
					array_map(
						'intval',
						CIntranetUtils::GetSubordinateDepartments(
