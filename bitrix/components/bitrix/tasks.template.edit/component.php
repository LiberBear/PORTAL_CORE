...
if (($arResult["ACTION"] == "edit" || $arResult["ACTION"] == "delete") && intval($arParams["TEMPLATE_ID"])) // check access rights and get data (better to rewrite the condition to: if(intval($arParams["TEMPLATE_ID"])) )
{
	...
	else
	{
		...
		$arReplicateParams = array(
			...
			"REPLICATE_TIME" => (string) $arData["REPLICATE_PARAMS"]["TIME"] != '' ? $arData["REPLICATE_PARAMS"]["TIME"] : '05:00',
		);
...
$arResult["USER_FIELDS"] = $GLOBALS["USER_FIELD_MANAGER"]->GetUserFields("TASKS_TASK_TEMPLATE", $arParams["TEMPLATE_ID"] ? $arParams["TEMPLATE_ID"] : 0, LANGUAGE_ID);
...
if($_SERVER["REQUEST_METHOD"] == "POST" && check_bitrix_sessid() && ($arResult["ACTION"] == "create" || $arResult["ACTION"] == "edit") && !isset($_POST['controller_id']))
{
	...
	if(isset($_POST["save"]) || isset($_POST["apply"]))
	{
		...
		else
		{
			...
			if ($arFields["CREATED_BY"] != $USER->GetID() && !$arResult['USER_IS_ADMIN'])
			{
		...
		if (sizeof($arResult["ERRORS"]) == 0)
		{
			...
			try
			{
				\Bitrix\Tasks\Template\CheckListTable::updateForTemplate($templateID, $checkList2Store);
			}
