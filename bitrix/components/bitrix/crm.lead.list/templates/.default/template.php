		...
		$resultItem = array(
			...
			'columns' => array(
				...
				'ASSIGNED_BY' => $arLead['~ASSIGNED_BY_ID'] > 0 ?
					...
					'<script type="text/javascript">BX.tooltip('.$arLead['~ASSIGNED_BY_ID'].', "balloon_'.$arResult['GRID_ID'].'_'.$arLead['ID'].'", "");</script>'
					: '',
	...
	$APPLICATION->IncludeComponent(
		...
		array(
			...
			'FILTER_NAVIGATION_BAR' => array(
				'ITEMS' => array(
					array('icon' => 'table', 'id' => 'list', 'active' => true, 'url' => $arParams['PATH_TO_LEAD_LIST']),
					array('icon' => 'chart', 'id' => 'widget', 'active' => false, 'url' => $arParams['PATH_TO_LEAD_WIDGET'])
				),
				'BINDING' => array(
