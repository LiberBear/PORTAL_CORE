...
if(isset($arResult['LEAD_ID']) && $arResult['LEAD_ID'] > 0)
{
	...
	$arFormButtons['custom_html'] = '<input type="hidden" name="company_id" value="'.$elementID.'"/><input type="hidden" name="lead_id" value="'.$arResult['LEAD_ID'].'"/>';
}
else
{
	$arFormButtons['standard_buttons'] = true;
	$arFormButtons['wizard_buttons'] = false;
	$arFormButtons['custom_html'] = '<input type="hidden" name="company_id" value="'.$elementID.'"/>';
}
