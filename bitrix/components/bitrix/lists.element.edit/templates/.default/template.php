...
if($arResult["CAN_DELETE_ELEMENT"])
{
	$arToolbar[] = array(
		...
		"LINK"=>"javascript:BX['".$jsClass."'].elementDelete('form_".$arResult["FORM_ID"]."',
...
foreach($arResult["FIELDS"] as $FIELD_ID => $arField)
{
		...
		if($arField["MULTIPLE"] == "Y" && $arField["TYPE"] != "S:DiskFile")
		{
			...
			if($read == "N")
			{
				if($checkHtml)
					...
					onclick="BX[\''.$jsClass.'\'].createAdditionalHtmlEditor(\'tbl'.$FIELD_ID.'\',
						\''.$FIELD_ID.'\', \''.$FIELD_ID.'\');">';
				else
					$html .= '<input type="button" value="'.GetMessage("CT_BLEE_ADD_BUTTON").'"
					onclick="BX[\''.$jsClass.'\'].addNewTableRow(\'tbl'.$FIELD_ID.'\', 1,
						/'.$FIELD_ID.'\[(n)([0-9]*)\]/g, 2)">';
			}
	...
	elseif($arField["PROPERTY_TYPE"] == "N")
	{
		...
		if($arField["MULTIPLE"] == "Y")
		{
			...
			if($read == "N")
				$html .= '<input type="button" value="'.GetMessage("CT_BLEE_ADD_BUTTON").'"
				onclick="BX[\''.$jsClass.'\'].addNewTableRow(\'tbl'.$FIELD_ID.'\', 1, /'.$FIELD_ID.'\[(n)([0-9]*)\]/g, 2)">';
		}
		else
		{
			if(!is_array($arResult["FORM_DATA"][$FIELD_ID]))
	...
	foreach ($arDocumentStates as $arDocumentState)
	{
		...
		if($canViewWorkflow)
		{
			...
			if(strlen($arDocumentState["ID"]) && CIBlockElementRights::UserHasRightTo($arResult["IBLOCK_ID"],
					$arResult["ELEMENT_ID"], "element_rights_edit") && strlen($arDocumentState["WORKFLOW_STATUS"]))
			{
				$arTab2Fields[] = array(
					...
					"value" => '<a href="javascript:void(0)"
						onclick="BX[\''.$jsClass.'\'].completeWorkflow(\''.$arDocumentState["ID"].'\',
...
<script type="text/javascript">
	BX(function () {
		BX['<?=$jsClass?>'] = new BX.ListsElementEditClass({
		...
		BX.message({
			LISTS_STATUS_ACTION_SUCCESS: '<?=GetMessageJS('LISTS_STATUS_ACTION_SUCCESS')?>',
			LISTS_STATUS_ACTION_ERROR: '<?=GetMessageJS('LISTS_STATUS_ACTION_ERROR')?>'
		});
