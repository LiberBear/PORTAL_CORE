...
class LiveFeedAjaxController extends Controller
{
	...
	protected function processActionCheckDataElementCreation()
	{
		...
		/* Attaching files disk */
		if($attachFileDisk && $idElement && CModule::includeModule("disk"))
		{
			$propertyValues = CLists::attachFileDisk(
				$this->iblockId,
				$idElement,
				$elementData["PROPERTY_VALUES"]
			);
			CIBlockElement::setPropertyValuesEx($idElement, $this->iblockId, $propertyValues);
		}
