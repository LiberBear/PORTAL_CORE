...
else
{
    ...
    $arSonetLog = \Bitrix\Tasks\Integration\Socialnetwork\Task::getSonetLogByTaskId($arResult['TEMPLATE_DATA']['TASK_ID']);
    $arContext = (
        !empty($arSonetLog)
            ? array(
                "ENTITY_TYPE" => "LOG_ENTRY",
                "ENTITY_ID" => $arSonetLog['ID']
            )
            : array()
    );
    ...
    foreach($formattedUsers as $i => $item)
    {
        ...
        $formattedUsers[$i]['URL'] = \Bitrix\Socialnetwork\ComponentHelper::addContextToUrl(CComponentEngine::MakePathFromTemplate($arParams["PATH_TO_USER_PROFILE"], array("user_id" => $item["ID"])), $arContext);
    ...
    if($arResult['TEMPLATE_DATA']['EDITABLE_OR_AUDITOR'])
    {
        $arResult['TEMPLATE_DATA']['ITEMS']['DATA'][] = array(
            'ID' =>                 '{{VALUE}}', // '{{ID}}',
            'NAME_FORMATTED' =>     '{{DISPLAY}}',// '{{NAME_FORMATTED}}',
            'WORK_POSITION' =>      '{{WORK_POSITION}}',
            'URL' =>                \Bitrix\Socialnetwork\ComponentHelper::addContextToUrl(CComponentEngine::MakePathFromTemplate($arParams["PATH_TO_USER_PROFILE"], array("user_id" => '{{VALUE}}')), $arContext),
            'AVATAR' =>             '{{AVATAR}}',
            'USER_TYPE' =>          '{{USER_TYPE}}',
        );
    }
