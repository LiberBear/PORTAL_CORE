;(function() {
	...
	BX.merge(BX.Mobile.Tasks.detail.prototype, {
		...
		init: function()
		{
			...
			BX.addCustomEvent("onTaskWasPerformed", BX.delegate(function(taskId, objectId, data) {
				if (!data)
				{
					data = taskId[2];
					objectId = taskId[1];
		...
		actExecute : function(errors, data) {
			...
			{
				window.app.onCustomEvent("onTaskWasRemoved", [this.task["ID"], this.variable("objectId"), data]);
			}
			else
			{
				window.app.onCustomEvent("onTaskWasPerformed", [this.task["ID"], this.variable("objectId"), data]);
